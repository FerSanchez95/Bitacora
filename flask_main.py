...
def exist_db(filename_path):
    ...
    if not os.path.isfile(filename_path): 
       return None
        ...
        existent_db = get_db(filename_path)
...
def custom_list(message):
    ...
    global transferList
    loaded_db = exist_db(filename)

    if loaded_db is None:
        db_loaded_message = "La base de datos no pudo ser cargada."
        transferList = []
        return transferList, db_loaded_message
    
    else:
        db_loaded_message = "La base de datos se a cargado correctamente."
        ...
        loaded_db.close()
        return reversed(transferList), db_loaded_message #La función 'reversed' se usa para que en la página se muestre la última entrada primero.
...
def index():
    ...
    if request.method == 'POST':
        ...
        lista_salida, db_info = custom_list(input_message)
        list_out = str(f"\n\r".join(lista_salida))
        print(lista_salida)
        flash(db_info)
